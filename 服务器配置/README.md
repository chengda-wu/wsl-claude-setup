...
- **`--override` 是 CUDA toolkit runfile 专属,driver runfile 不认**: 把 `--override` 传给 `NVIDIA-Linux-*.run`(驱动,内部是 nvidia-installer)会报 `unrecognized option --override`。`--override` 只在 `cuda_*.run`(toolkit,绕过编译器版本检查)有效。若不想对支持列表外(如 20.04,610 官方支持 22.04/24.04/26.04)的 driver 硬装，需另想办法，`--override` 对此无效。
- **CUDA toolkit runfile 的参数跟 driver runfile 不同**: 实测 `cuda_*.run --help`,toolkit runfile 认 `--silent --toolkit --override --no-man-page`(**无 `--accept-license`、无 `--no-man-page-install`**)。`--silent` 已隐含接受 EULA;man page 正确拼写是 `--no-man-page`。用 `--accept-license`/`--no-man-page-install` 会报 unknown option。
- **`nvidia-smi` 的 `CUDA UMD Version: 13.3` 与开发环境无关**: 那是**驱动自带**的用户态驱动(CUDA UMD)版本,由驱动(610.57.04)决定,显示 13.3 不表示你的 toolkit 是 13.3。开发环境看 `nvcc --version`(应为 13.0)。两不相干,toolkit 是 `--toolkit` 独立装的,框架 cu130 wheel 用的是 nvcc/toolkit 13.0,不受 UMD 13.3 影响。
- **多卡时 `nvidia-smi ... | head -1` 会被 set -e 杀掉**: 8 卡时 nvidia-smi 输出 8 行,`head -1` 提前关管道让 nvidia-smi 收到 SIGPIPE(退出码 141),脚本若开 `set -o pipefail` 整条管道返回非 0 → 被 `set -e` 静默退出,且不打印任何报错(表现为驱动装好后脚本在"验证驱动"步莫名退出)。处理:管道加 `|| true`。(曾因此修了 4 处。)
